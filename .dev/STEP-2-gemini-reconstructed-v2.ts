// ============================================================================
// gemini-v2.ts - ENHANCED VERSION of STEP-2-gemini-reconstructed.ts
// Captures current state: modular refactoring + new provider architecture
// ============================================================================
// NEW CAPABILITIES ADDED (not in original):
//   - GeminiProvider class implementing ConversationProvider interface
//   - Multi-account detection (detectAccounts scans up to 10 account indices)
//   - Explicit auth lifecycle: refreshAuth(), isAuthenticated(), clearProfileCache()
//   - ProviderConfig with capability declarations (8 capabilities)
//   - Lifecycle hooks: init() / destroy()
//   - handleNetworkActivity returns SyncTrigger (instead of callback-based onFetch)
//   - ensureToken() extracted helper for DRY token-refresh pattern
//   - OrgStatus enum (replaces idb.orgs.status magic strings)
//   - New types: AuthProfile, PaginatedResult, SearchResult, RequestDetails, SyncTrigger
//   - hasCapability() runtime capability check
//   - Email-based account IDs for stability (vs raw SID cookies)
// ============================================================================
...
import {
  ServiceDefinition, Conversation, Message, Account, Org, Header,
  AuthProfile, PaginatedResult, SearchResult, RequestDetails, SyncTrigger,
  ProviderConfig, ProviderCapability, OrgStatus,
} from '../types'
...
// ============================================================================
// 2. Service Definition (exported as "gemini" — backward-compatible)
// ============================================================================
...
// ============================================================================
// 2b. ProviderConfig — NEW: capability declarations for the provider registry
// ============================================================================

export const geminiProviderConfig: ProviderConfig = {
  id: SERVICE_ID,
  name: 'Gemini',
  domain: GEMINI_DOMAIN,
...
// ============================================================================
// 2c. GeminiProvider class — NEW: implements ConversationProvider interface
//     This is the modern entry point. The legacy geminiService object above
//     remains for backward compatibility with older extension code.
// ============================================================================
...
export class GeminiProvider {
  ...
  /**
   * NEW: Multi-account detection — scans up to 10 account indices,
   * extracts profiles, and creates/updates local Account + Org records.
   * Uses email-based IDs for stability (vs raw SID cookies).
   */
  async detectAccounts(): Promise<Account[]> {
    ...
    for (let i = 0; i < maxAccounts; i++) {
      try {
        ...
        // Email-based ID for stability and backward compatibility
  ...
  /** NEW: Explicit auth refresh for a given account */
  ...
  async getConversation(account: Account, conversationId: string): Promise<Conversation | null> {
    ...
    const conversation = header
      ? { ...header, accountId: account.id, orgId: account.id }
      : { id: conversationId, accountId: account.id, orgId: account.id }
  ...
  /**
   * NEW: Returns SyncTrigger instead of calling a callback.
   * The caller (sync manager) decides what to do with the trigger.
   */
...
// ============================================================================
// 3. Account Fetch Helpers — exported for backward compatibility
// ============================================================================
...
// ============================================================================
// 5. handleNetworkActivity — NEW: returns SyncTrigger (replaces callback-based onFetch)
//    The original onFetch is preserved below for backward compatibility.
// ============================================================================

async function handleNetworkActivity(details: RequestDetails): Promise<SyncTrigger | null> {
  ...
  if (!shouldSync) {
    ...
    const profile = extractProfileInfo('o30O0e', details.response?.body)
  ...
  // NEW: Return SyncTrigger instead of calling callback
  return {
...
// ============================================================================
// 5b. onFetch — LEGACY: callback-based version (preserved for backward compat)
// ============================================================================

async function onFetch(requestDetails: any, triggerSync: (org: any) => Promise<void>): Promise<void> {
  const trigger = await handleNetworkActivity(requestDetails)
  if (!trigger) return

  // Map SyncTrigger back to legacy callback behavior
  const { settings } = await LiveStorage.get({
    settings: { general: { manualSync: LiveStorage.defaultValue } },
  })
...
/** NEW: Explicitly clear the profile cache (called on destroy / rate limit reset) */
...
// ============================================================================
// 11b. ensureToken — NEW: DRY helper for token-refresh pattern
//     Extracted from the repeated { index, token } refresh logic in edit/delete/fetch.
// ============================================================================
...
// ============================================================================
// 13b. fetchHeadersInternal — NEW: Provider-facing version that accepts Account
// ============================================================================
...
// ============================================================================
// 15b. fetchContentInternal — NEW: Provider-facing version accepting Account
// ============================================================================
...
// ============================================================================
// 18b. createConversationInternal — NEW: Provider-facing version
// ============================================================================
...
// ============================================================================
// 19b. fetchSummaryInternal — NEW: Provider-facing version
// ============================================================================
...
async function fetchSummaryInternal(
  ...
  try {
    ...
    try {
      ...
      // Best-effort cleanup — don't fail the summary if delete fails
    }
...
// ============================================================================
// 20b. searchGeminiConversationsInternal — NEW: Provider-facing version
// ============================================================================
...
// ============================================================================
// 22. NEW: Internal mutation helpers for GeminiProvider class
// ============================================================================
